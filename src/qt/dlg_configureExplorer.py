...
class ConfigureExplorerServers_dlg(QDialog):
    ...
    def insert_server_list(self, server, index):
        id = server['id']
        server_line = QWidget()
    ...
    def loadServers(self):
        ...
        # Fill serversBox. Index by list position (explorerServersList holds
        # both networks; the header dropdown is filtered, so its indices differ).
        self.serverItems = {}
        for index, server in enumerate(self.main_wnd.explorerServersList):
            self.insert_server_list(server, index)
